...
def get_purchased_tickets_handler(repository: TicketRepositoryImpl = Depends(get_ticket_repository)) -> GetPurchasedTicketsHandler:
    return GetPurchasedTicketsHandler(repository)
...
def get_sales_report_handler(query_service = Depends(get_sales_query_service)) -> GetEventSalesReportHandler:
...
def get_reject_refund_handler(refund_repo = Depends(get_refund_repository)) -> RejectRefundHandler:
    return RejectRefundHandler(refund_repo)

def get_mark_paid_out_handler(refund_repo = Depends(get_refund_repository)) -> MarkRefundPaidOutHandler:
